...
# Update Task 4 charts data
df_trend_filtered = df_trend[df_trend['token'].isin(df_filtered['token.symbol'].unique())]
df_supply_filtered = df_supply[df_supply['token.symbol'].isin(df_filtered['token.symbol'].unique())]
df_top_filtered = df_top[df_top['token.symbol'].isin(df_filtered['token.symbol'].unique())]
...
with col4:
    ...
    for token in df_trend_filtered['token'].unique():
        token_df = df_trend_filtered[df_trend_filtered['token'] == token]
...
with col5:
    ...
    for token in df_supply_filtered['token.symbol'].unique():
        token_df = df_supply_filtered[df_supply_filtered['token.symbol'] == token]
...
bars = ax6.barh(df_top_filtered['token.symbol'], df_top_filtered['total_transferred'], color='orange')
